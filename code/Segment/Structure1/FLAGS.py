...
def CHECK():
    assert isinstance(DATA.TRAIN.LABEL_FOLDER,str)
    assert isinstance(DATA.TRAIN.IMAGE_FOLDER,str)
    assert isinstance(DATA.TRAIN.VALIDATION_SPLIT_RATIO,float)
    assert isinstance(DATA.TRAIN.VALIDATION_SPLIT_RANDOM_STATE,int)
    assert isinstance(DATA.TRAIN.BATCH_SIZE,int)
